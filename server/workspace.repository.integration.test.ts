...
describe("SentiX workspace repository persistence", () => {
  databaseTest("persists and reopens reviews and prompt history through the real helper layer", async () => {
    ...
    try {
      await replaceSentiXWorkspaceReviews(userId, workspace.id, [{ id: "integration-rv-1", text: "The refund process took too long.", source: "integration.xlsx", rating: 1, category: "Refunds", label: "Negative", compound: -0.74, confidence: 94, vaderCompound: -0.72, transformerConfidence: 92, transformerUsed: true, actionTag: "Investigate refund workflow", timestamp: 1_726_000_000_000 }]);
      ...
      expect(restored.reviews[0]).toMatchObject({ id: "integration-rv-1", category: "Refunds", compound: -0.74 });

...
export default function SentiXDashboard({ embedded = false }: { embedded?: boolean }) {
  ...
  const importFile = async (file?: File) => {
    ...
    try {
      ...
      setImportProgress(`Found ${drafts.length} complete review${drafts.length === 1 ? "" : "s"}. Loading into analyzer…`);
  ...
  return <div className={embedded ? "min-h-screen app-dashboard fine-grid" : "min-h-screen fine-grid"}>
    ...
    <Button aria-label="Ask SentiX AI" onClick={() => void openChat()} className="ask-sentix-live fixed bottom-6 right-6 z-20 h-12 rounded-full bg-indigo-500 px-5 text-white shadow-lg shadow-indigo-950/30 transition hover:bg-indigo-400"><MessageSquareText className="mr-2 h-4 w-4" />Ask SentiX AI</Button>

...
import { Activity, Bot, Check, CloudUpload, Download, FileDown, FileText, Inbox, Loader2, MessageSquareText, Moon, Pencil, Plus, Search, Sparkles, Sun, Upload, WandSparkles, X } from "lucide-react";
...
export default function SentiXDashboard() {
  ...
  return <div className="min-h-screen fine-grid">
    ...
    <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-5"><div className="glass-panel rounded-2xl border border-slate-700/80 p-5 xl:col-span-2"><p className="font-bold text-slate-100">Sentiment breakout</p>{metrics.total ? <div className="mt-3 h-56"><ResponsiveContainer><PieChart><Pie data={donut} dataKey="value" nameKey="name" innerRadius={55} outerRadius={82} paddingAngle={5}>{donut.map(item => <Cell key={item.name} fill={COLORS[item.name as Sentiment]} />)}</Pie><Legend /></PieChart></ResponsiveContainer></div> : <p className="mt-16 text-center text-sm text-slate-500">Awaiting business-review evidence.</p>}</div><div className="glass-panel rounded-2xl border border-slate-700/80 p-5 xl:col-span-3"><p className="font-bold text-slate-100">Polarity trend</p>{trend.length ? <div className="mt-3 h-56"><ResponsiveContainer><LineChart data={trend}><XAxis dataKey="step" /><YAxis domain={[-1, 1]} /><RechartsTooltip /><Line type="monotone" dataKey="polarity" stroke="#818cf8" strokeWidth={3} /></LineChart></ResponsiveContainer></div> : <p className="mt-16 text-center text-sm text-slate-500">Analyze business feedback to populate the trend.</p>}</div></section>
